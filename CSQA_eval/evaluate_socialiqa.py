...
from dataset_eval_socialiqa import *
...
def parse_option():
    ...
    parser.add_argument('--testfile', type=str, default='../dataset/SIQA/socialiqa-train-dev/dev.jsonl', \
                        help='test file path')
...
def eval(eval_loader, model, tokenizer, criterion, opt):
    ...
    if 'test' in opt.testfile:
        fw = open(os.path.join(opt.save_folder, 'socialiqa_test_pred.txt'), 'w')
    elif 'dev' in opt.testfile:
        fw = open(os.path.join(opt.save_folder, 'socialiqa_dev_pred.txt'), 'w')
...
def main():
    ...
    if 'test' in opt.testfile:
        raw_test_data = read_test_jsonl(opt.testfile)
    else:
        raw_test_data = read_jsonl(opt.testfile, opt.testlabel)
